...
p <- as.data.frame(loadings) %>%
  ...
  select(Pathway, PC1) %>%
  full_join(levels) %>%
  ggplot(aes(x = desc(reorder(Pathway, PC1)), y = PC1, fill = Pathway_Level1, color = Pathway_Level1)) +
...
p <- as.data.frame(loadings) %>%
  ...
  select(Pathway, PC2) %>%
  full_join(levels) %>%
  ggplot(aes(x = desc(reorder(Pathway, PC2)), y = PC2, fill = Pathway_Level1, color = Pathway_Level1)) +
  geom_bar(stat = "identity", alpha =0.8, size =0.3, width = 0.4)+ coord_flip() +
  scale_fill_brewer(palette = "Dark2", name = "Pathway group")+
  scale_color_brewer(palette = "Dark2", name = "Pathway group")+
  theme_classic() +
...
p <- as.data.frame(loadings) %>%
  ...
  select(Pathway, PC3) %>%
  full_join(levels) %>%
  ggplot(aes(x = desc(reorder(Pathway, PC3)), y = PC3, fill = Pathway_Level1, color = Pathway_Level1)) +
  geom_bar(stat = "identity", alpha =0.8, size =0.3, width = 0.4)+ coord_flip() +
  scale_fill_brewer(palette = "Dark2", name = "Pathway group")+
  scale_color_brewer(palette = "Dark2", name = "Pathway group")+
  theme_classic() +
